...
task :setup_acceptance_db do
    ...
    # The cache can then be checked into the repo to speed up future test runs.
    else
    ...
    # Update the cache.  If this commit introduced new migrations or models,
    # then the cache will appear changed in the git status.
    # Developers can then commit the updated cache, so when it gets
    # merged the migrations will be included in future commits.
